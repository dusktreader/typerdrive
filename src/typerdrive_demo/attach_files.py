...
def demo_1__attach_files__storing_data():
    ...
    def store(ctx: typer.Context, manager: FilesManager):  # pyright: ignore[reportUnusedFunction]
        # Store text files
...
def demo_2__attach_files__loading_data():
    ...
    @cli.command()
    @attach_files(show=True)
    def load(ctx: typer.Context, manager: FilesManager):  # pyright: ignore[reportUnusedFunction]
        # Load text
...
def demo_3__attach_files__with_permissions():
    ...
    @cli.command()
    @attach_files(show=True)
    def store_secret(ctx: typer.Context, manager: FilesManager):  # pyright: ignore[reportUnusedFunction]
...
def demo_4__attach_files__list_and_manage():
    ...
    @cli.command()
    @attach_files()
    def list_templates(ctx: typer.Context, manager: FilesManager):  # pyright: ignore[reportUnusedFunction]
        # List all templates
...
def demo_5__attach_files__access_through_context():
    ...
    @cli.command()
    @attach_files(show=True)
    def store(ctx: typer.Context):  # pyright: ignore[reportUnusedFunction]
